...
{
    /* transmission completed, start receiving again */
    DMA_CCR_t ccr =
    {
        .bits.EN = 1,
        .bits.TCIE = 1,
    ...
    DMA_PADDR(DMA1_BASE, USART_RX_DMA_CHANNEL, &USART_DR(USART1_BASE));
    DMA_MADDR(DMA1_BASE, USART_RX_DMA_CHANNEL, rxbuf);
    DMA_TRANSFER_NUM(DMA1_BASE, USART_RX_DMA_CHANNEL, sizeof(rxbuf));
    USART_TX_COMPLETE_CLEAR(USART1_BASE);
    DMA_CCR(DMA1_BASE, USART_RX_DMA_CHANNEL) = ccr.u32;
}

void rx_complete(void)
{
    /* reception completed, send data back */
    const int len =
        snprintf(
            txbuf, sizeof(txbuf),
            "Hello USART RX DMA %010u [%.8s]\n", tx_cntr, rxbuf);
    ++tx_cntr;
    ...
    DMA_MADDR(DMA1_BASE, USART_TX_DMA_CHANNEL, txbuf);
...
void isrDMA1_Channel5(void)
{
    DMA_DISABLE(DMA1_BASE, USART_RX_DMA_CHANNEL);
    DMA_COMPLETE_INT_CLEAR(DMA1_BASE, USART_RX_DMA_CHANNEL);
    DMA_ERROR_INT_CLEAR(DMA1_BASE, USART_RX_DMA_CHANNEL);
    rx_complete();
}
...
void main(void)
{
    ...
    USART_RX_DMA_ENABLE(USART1_BASE);
    ISR_ENABLE(isrNoDMA1_Channel4);
    ISR_ENABLE(isrNoDMA1_Channel5);
    tx_complete();
